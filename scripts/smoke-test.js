...
test('托盘菜单来自共用模型(项序/分组单一来源),弹出前重建保证勾选不过期', () => {
  ...
  // ⚠️ 自启项【不得】传 name:Windows 下 get 判定 openAtLogin 时固定读 AppUserModelId 值名,
  // options.name 被忽略 ⇒ 传 name 会让状态永远读回 false、还让"清理"逻辑误删自启项(1.1.4 根因)。
  assert.ok(!/\bAUTOSTART_REG_NAME\b/.test(main), 'setLoginItemSettings 不应再传 name(读写口径必须同为 AppUserModelId)');
  assert.ok(main.includes('LEGACY_AUTOSTART_REG_NAME'), '应保留历史自启值名常量用于迁移');
  assert.ok(main.includes('migrateAutostart'), '应迁移历史遗留的重复自启项');
  // 两个入口共用同一动作分发入口
